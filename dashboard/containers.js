  ...
  async function renderContainers() {
    ...
    try {
      ...
      host.innerHTML = `
        ...
        <section class="card" style="margin-bottom:16px">
          ...
          <div class="orchestrator-form" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px">
            <label>Image <input id="containers-image" class="input" value="${esc(localStorage.getItem('containers_image') || 'nucleusdb:latest')}"></label>
      ...
      document.querySelector('#containers-provision-btn')?.addEventListener('click', async () => {
        try {
          const image = String(document.querySelector('#containers-image')?.value || 'nucleusdb:latest').trim();
