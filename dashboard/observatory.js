...
(function() {
  ...
  ObservatoryDrawer.prototype.pushData = function(vizType, data) {
    ...
    if (vizType === 'codefile' && data && data.path) {
      ...
      var sessionEntry = tracking[this.panelId] || {};
      var files = Array.isArray(sessionEntry.files) ? sessionEntry.files : [];
      var fileEntry = {
        path: data.path,
      ...
      sessionEntry.lastPath = data.path;
      sessionEntry.lastUpdated = fileEntry.timestamp;
      sessionEntry.files = [fileEntry].concat(files.filter(function(entry) {
        return entry && entry.path !== data.path;
      })).slice(0, 20);
      tracking[this.panelId] = sessionEntry;
  ...
  function disposeVizContainer(container) {
    if (!container) return;
    if (container._monacoDiffEditor) {
      try { container._monacoDiffEditor.dispose(); } catch (_e) {}
      container._monacoDiffEditor = null;
    }
    if (container._monacoEditor) {
      try { container._monacoEditor.dispose(); } catch (_e) {}
      container._monacoEditor = null;
    }
    if (Array.isArray(container._monacoModels)) {
      container._monacoModels.forEach(function(model) {
        if (!model) return;
        try { model.dispose(); } catch (_e) {}
      });
    }
    container._monacoModels = null;
  }
  ...
  function renderCodefile(data, container) {
    ...
    var locked = !!container._locked;
    header.innerHTML =
      '<span class="obs-code-path">' + esc(data.path || 'untitled') + '</span>' +
      '<button class="obs-code-lock" title="Lock: auto-follow agent edits">' +
  ...
  function renderDiffEditor(data, container) {
    ...
    container._monacoDiffEditor = diffEditor;
    container._monacoModels = [originalModel, modifiedModel];
